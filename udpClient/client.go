...
func initializeMembers(ip string) (map[string]Entry, string) {
	...
	memberAddr, err := net.ResolveUDPAddr("udp", CONTACT_POINT)
	logError(err)
	//
...
func sendHeartBeat(members map[string]Entry, selfName string) {
	...
	b, err := json.Marshal(members)
	fmt.Println("printing before sending")
	for member, _ := range members {
		fmt.Print(member)
		fmt.Print("   ")
		fmt.Println(members[member])
	...
	for i := range kMembers {
		...
		memberIp := a[1]
		//retrieve a UDPaddr
		memberAddr, err := net.ResolveUDPAddr("udp", memberIp+":"+"8000")
		logError(err)
		//
		conn, err := net.DialUDP("udp", nil, memberAddr)
		if !logError(err) {
